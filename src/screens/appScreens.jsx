...
export function HomeSearchScreen({ go, clients, homeSearch, setHomeSearch }) {
...
export function GlobalSearchScreen({ go, ctx, clients, chats, docs }) {
...
// HOME SCREEN
// ══════════════════════════════════════════════════════════════════
export function HomeScreen({ go, clients, signatures, forms8879, extensions, reminded, remind, undoRemind, showToast, hideToast, setOverlayOpen, signed, homeSearch, showScrollSearch }) {
  ...
  const [aiPrompt, setAiPrompt] = useState("What should I prioritize today?");
  ...
  const aiPromptPresets = [
    ...
    "Show clients most at risk",
    "Where can I unblock work fastest?",
  ];

  const runAiInsight = () => {
    const atRiskNow = scopedClients.filter((c) => c.risk === "high" || c.urgent).length;
    const blockedNow = scopedClients.filter((c) => c.blockedBy === "client").length;
    const reviewNow = scopedClients.filter((c) => c.step >= 5 && c.step <= 6).length;

    if (aiPrompt.includes("risk")) {
      showToast(`AI insight: ${atRiskNow} at-risk clients. Start with critical blockers in Focus Queue.`);
      return;
    }
    if (aiPrompt.includes("unblock")) {
      showToast(`AI insight: ${blockedNow} client-blocked files. Send reminders first to unlock fastest throughput.`);
      return;
    }
    showToast(`AI insight: Prioritize ${reviewNow} returns in review/signing and clear critical blockers first.`);
  };
  ...
  return (
    <div style={{ flex:1 }}>
      ...
      <div style={{ padding:"8px 18px 20px" }}>
        ...
        <div style={{ marginTop:22, marginBottom:72 }}>
          <div style={{ fontSize:14, fontWeight:700, color:"#666666", letterSpacing:0.4, marginBottom:8 }}>Ask AI</div>
          <IOSCard padding="16px 12px 12px">
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center" }}>
              ...
              <div style={{ marginTop:10, fontSize:14, fontWeight:600, color:"#3b4354", textAlign:"center" }}>
              ...
              <div style={{ width:"100%", display:"flex", flexDirection:"column", alignItems:"center", gap:7, marginTop:10 }}>
                {aiPromptPresets.map((preset, idx) => (
                  <button
                    ...
                    style={{
                      width: idx === 0 ? "100%" : idx === 1 ? "95%" : "84%",
                      border:"1px solid #e0e5f0",
                      borderRadius:999,
                      background: aiPrompt === preset ? "#edf3ff" : "#fff",
                      color: aiPrompt === preset ? "#2f4d8f" : "#687081",
                      fontSize:10.8,
                      fontWeight:600,
                      padding:"8px 10px",
                      cursor:"pointer",
            ...
            <form
              ...
              style={{ marginTop:20 }}
            >
              <div style={{ border:"1px solid #dbe2f0", borderRadius:14, background:"#fff", padding:"4px", display:"flex", alignItems:"center", gap:6 }}>
                <input
                  value={aiPrompt}
...
// ══════════════════════════════════════════════════════════════════
export function ClientsScreen({ go, clients, reminded, remind }) {
  ...
  const staffingStageOrder = ["Preparation", "Awaiting Client Info", "Info Received", "QC Review", "Review", "Assembly", "Signer Sign-Off"];
  const staffingStageColors = {
    Preparation:"#6b7280",
    "Awaiting Client Info":"#4cb3e3",
    "Info Received":"#22c55e",
    "QC Review":"#f59e0b",
    Review:"#8b5cf6",
    Assembly:"#6366f1",
    "Signer Sign-Off":"#ef4444",
  };
  const stageFromStep = (step) => staffingStageOrder[Math.max(0, Math.min((step || 1) - 1, staffingStageOrder.length - 1))];
  const clientsByStage = staffingStageOrder
    .map((stage) => ({ stage, clients: filtered.filter((c) => stageFromStep(c.step) === stage) }))
  ...
  const ClientCardClient = ({ c }) => (
    <div onClick={() => go(S.CLIENT_DETAIL, { client:c })} style={{ background:"#fff", border:`1px solid ${c.urgent?"#c8c8c8":"#d8d8d8"}`, borderRadius:14, padding:13, marginBottom:10, cursor:"pointer" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
        ...
        <div style={{ display:"flex", alignItems:"center" }}><CardChevron /></div>
      </div>
      {/* Progress */}
      <div>
      ...
      <div style={{ padding:"8px 18px 20px" }}>
        {view === "client" ? (
          clientsByStage.length > 0 ? (
            clientsByStage.map((group) => (
              <div key={group.stage} style={{ marginBottom:12 }}>
                <div style={{ display:"flex", alignItems:"center", marginBottom:8 }}>
                  <span style={{ fontSize:11.5, fontWeight:700, color:"#666666", letterSpacing:0.35 }}>{group.stage}</span>
                </div>
                {group.clients.map((c) => <ClientCardClient key={c.id} c={c} />)}
              </div>
            ))
          ) : (
            <IOSCard padding="14px 12px">
              <div style={{ fontSize:12, color:"#7b8393" }}>No clients found for this filter.</div>
            </IOSCard>
          )
        ) : (
          <>
...
export function ClientDetailScreen({ go, ctx, docs, chats, extensions, reminded, remind }) {
  ...
  const contactById = {
    ...
    3:{ email:"tiffanytrust@mail.com", phone:"+1 (303)-555-0187" },
  ...
  const responsivenessById = {
    ...
    2: { status:"Watch", avgDays:4, bars:[2,3,2,2,1] },
    3: { status:"Stable", avgDays:2, bars:[2,3,3,2,3] },
    4: { status:"At risk", avgDays:6, bars:[1,1,2,1,1] },
    5: { status:"Watch", avgDays:3, bars:[2,3,2,2,2] },
  ...
  const statusTone = responsiveness.status === "Stable"
    ? { bg:"#f7f2e5", color:"#b58f00", border:"#f0e3bd" }
    : responsiveness.status === "Watch"
      ? { bg:"#f7efe5", color:"#a97732", border:"#ecd9be" }
      : { bg:"#fdecec", color:"#b14646", border:"#f1cccc" };
        ...
        {detailTab === "Notes" && (
          <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:12 }}>
            {clientReviewNotes.map((note) => {
              ...
              return (
                <div key={note.id} style={{ background:"#fff", border:"1px solid #e6e9ef", borderRadius:12, padding:"10px 12px" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                      <span style={{ fontSize:10, fontWeight:700, color:"#7c8494", textTransform:"uppercase" }}>{note.id}</span>
                      <span style={{ fontSize:10, fontWeight:700, color:"#0f766e", background:"#e9f6f3", border:"1px solid #bde9df", borderRadius:7, padding:"2px 6px" }}>{note.source}</span>
                    </div>
                    <span style={{ fontSize:10.5, fontWeight:700, color:isOpen ? "#187d3f" : "#6d7586", background:isOpen ? "#ecf9f1" : "#f2f4f8", border:`1px solid ${isOpen ? "#c7ebd4" : "#dde2ea"}`, borderRadius:999, padding:"2px 8px" }}>
                      {note.status}
                    </span>
                  </div>
                  <div style={{ fontSize:13, fontWeight:600, color:"#263042", lineHeight:1.35, marginTop:8 }}>
                    {note.description}
                  </div>
                  <div style={{ fontSize:11, color:"#6f7788", marginTop:6, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                    {note.reference}
                  </div>
                  {note.latestResponse && (
                    <div style={{ fontSize:11.5, color:"#5c6578", marginTop:4, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                      Latest: {note.latestResponse}
                    </div>
                  )}
                </div>
              );
            })}
            {clientReviewNotes.length === 0 && (
              <div style={{ background:"#fff", border:"1px solid #e6e9ef", borderRadius:12, padding:"12px", fontSize:12, color:"#8a94a7" }}>
                No review notes yet.
              </div>
            )}
            <button onClick={openClientChat} style={{ border:"none", borderRadius:12, padding:"10px", marginTop:2, width:"100%", fontSize:12, fontWeight:700, background:"#0065FF", color:"#fff", cursor:"pointer" }}>Open conversation</button>
          </div>
        )}
...
// EL STATUS SCREEN
// ══════════════════════════════════════════════════════════════════
export function ELStatusScreen({ go, clients, reminded, remind, showToast }) {
...
// EL WIZARD SCREEN
// ══════════════════════════════════════════════════════════════════
export function ELWizardScreen({ go, ctx, clients, showToast }) {
...
// ══════════════════════════════════════════════════════════════════
export function MySignaturesScreen({ go, signatures, forms8879, signed }) {
...
// SIGN DOC SCREEN
// ══════════════════════════════════════════════════════════════════
export function SignDocScreen({ go, ctx, sign, signed }) {
...
// 8879 SCREEN
// ══════════════════════════════════════════════════════════════════
export function Form8879Screen({ go, forms8879, reminded, remind, sign, signed }) {
  const [tab, setTab] = useState("All");
...
// BATCH EXTENSIONS
// ══════════════════════════════════════════════════════════════════
export function BatchExtScreen({ go, extensions, clients }) {
  return (
    <div style={{ flex:1 }}>
...
// ── Batch Ext Status ──────────────────────────────────────────────
export function BatchExtStatusScreen({ go, extensions, ctx, reminded, remind }) {
...
// TAX RETURN PROGRESS
// ── Audit Log ─────────────────────────────────────────────────────
export function AuditLogScreen({ go, ctx, auditLog }) {
  const c = ctx.client;
  return (
    <div style={{ flex:1 }}>
...
// ══════════════════════════════════════════════════════════════════
export function ChatTopicsScreen({ go, ctx, chatMsgs, openTopic }) {
      ...
      <div style={{ background:UI.brand, padding:"12px 16px 0", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, paddingBottom:12 }}>
          ...
          <div style={{ position:"relative" }}>
            <div style={{ width:40, height:40, borderRadius: isAI ? 12 : 99, background: isAI ? "rgba(217,230,255,0.32)" : "rgba(255,255,255,0.18)", color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700, fontSize:15 }}>{isAI ? "AI" : chat.name[0]}</div>
            {!isAI && chat.online && <div style={{ position:"absolute", bottom:1, right:1, width:9, height:9, borderRadius:99, background:"#8a8a8a", border:`2px solid ${UI.brand}` }} />}
          </div>
          <div style={{ flex:1 }}>
            <div style={{ color:"#fff", fontWeight:700, fontSize:15 }}>{chat.name}</div>
            <div style={{ color:"#b0b0b0", fontSize:10 }}>{isAI ? "AI assistant thread" : `${topics.length} topics · ${chat.type}`}</div>
          </div>
          <HeaderIcons />
        </div>
...
// MESSAGING SCREEN
// ══════════════════════════════════════════════════════════════════
export function MessagingScreen({ go, chats, chatUnread, chatMsgs, openChat }) {
  const [q, setQ] = useState("");
  const filtered = chats.filter(c => c.name.toLowerCase().includes(q.toLowerCase()));

  const aiChats = filtered.filter(c => c.type === "AI");
  const clientChats = filtered.filter(c => c.type !== "AI");
    ...
    return (
      <div onClick={() => openChat(chat)} style={{ display:"flex", alignItems:"center", gap:11, padding:"12px 18px", borderBottom:"1px solid #f2f2f2", cursor:"pointer", background:"#fff" }}>
        <div style={{ position:"relative", flexShrink:0 }}>
          <div style={{ width:44, height:44, borderRadius: isAI ? 12 : 99, background: isAI ? "#dbe8ff" : "#2d2d2d", color: isAI ? "#1e66f5" : "#fff", border: isAI ? "1px solid #c8dafd" : "none", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700, fontSize:16 }}>{isAI ? "AI" : chat.name[0]}</div>
          {!isAI && chat.online && <div style={{ position:"absolute", bottom:1, right:1, width:10, height:10, borderRadius:99, background:"#8a8a8a", border:"2px solid #fff" }} />}
        </div>
        <div style={{ flex:1, minWidth:0 }}>
  ...
  return (
    <div style={{ flex:1 }}>
      ...
      <div style={{ paddingTop:12 }}>
        {aiChats.length > 0 && (
          <>
            <div style={{ fontSize:10, fontWeight:700, color:"#999999", letterSpacing:1.5, padding:"0 18px 4px" }}>AI</div>
            {aiChats.map(c => <ChatRow key={c.id} chat={c} />)}
          </>
        )}
        <div style={{ fontSize:10, fontWeight:700, color:"#999999", letterSpacing:1.5, padding: aiChats.length > 0 ? "10px 18px 4px" : "0 18px 4px" }}>CLIENTS</div>
...
// ── Chat Screen ───────────────────────────────────────────────────
export function ChatScreen({ go, ctx, chatMsgs, sendMsg }) {
  ...
  const msgs = chatMsgs[key] || topic?.msgs || [];
  const handle = () => { if (!input.trim()) return; sendMsg(key, input.trim(), chat); setInput(""); };
  const quick = isAI
    ? ["What should I prioritize today?","Show clients most at risk","Where can I unblock work fastest?","Summarize staffing load"]
    : ["Documents received","Please upload missing docs","Return ready for review","Reminder: sign 8879"];
  ...
  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ background:UI.brand, padding:"12px 16px", display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
        ...
        <div style={{ position:"relative" }}>
          <div style={{ width:36, height:36, borderRadius: isAI ? 10 : 99, background: isAI ? "rgba(217,230,255,0.3)" : "rgba(255,255,255,0.2)", color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700, fontSize:14 }}>{isAI ? "AI" : chat.name[0]}</div>
          {!isAI && chat.online && <div style={{ position:"absolute", bottom:1, right:1, width:9, height:9, borderRadius:99, background:"#8a8a8a", border:`2px solid ${UI.brand}` }} />}
        </div>
        <div style={{ flex:1 }}>
          <div style={{ color:"#fff", fontWeight:700, fontSize:14 }}>{chat.name}</div>
          <div style={{ color:"#b0b0b0", fontSize:10 }}>{isAI ? "AI assistant" : (topic ? `${topic.icon} ${topic.label}` : (chat.online?"online":"last seen recently"))}</div>
        </div>
        <HeaderIcons />
      </div>
        ...
        {msgs.map((m, i) => {
          ...
          return (
            <div key={m.id} style={{ display:"flex", justifyContent: isCpa?"flex-end":"flex-start", marginTop: prev?1:6 }}>
              {!isCpa && !prev && <div style={{ width:26, height:26, borderRadius:99, background: isAiIncoming ? "#dbe8ff" : "#2d2d2d", color: isAiIncoming ? "#1e66f5" : "#fff", border: isAiIncoming ? "1px solid #c8dafd" : "none", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, marginRight:5, flexShrink:0, alignSelf:"flex-end" }}>{isAiIncoming ? "AI" : chat.name[0]}</div>}
              {!isCpa && prev  && <div style={{ width:31 }} />}
              <div style={{ maxWidth:"72%", background: isCpa?"#0065FF":(isAiIncoming ? "#eaf1ff" : "#fff"), color: isCpa?"#fff":(isAiIncoming ? "#1f3f87" : "#1a1a1a"), borderRadius: isCpa?(prev?"16px 4px 16px 16px":"16px 4px 4px 16px"):(prev?"4px 16px 16px 16px":"4px 16px 16px 4px"), padding:"7px 11px", boxShadow:"0 1px 2px rgba(0,0,0,0.07)" }}>
      ...
      <div style={{ background:"#fff", padding:"8px 12px 12px", display:"flex", alignItems:"flex-end", gap:8, flexShrink:0 }}>
        <div style={{ flex:1, background:"#f2f2f2", borderRadius:20, display:"flex", alignItems:"center", padding:"7px 12px", gap:6 }}>
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key==="Enter" && handle()} placeholder={isAI ? "Ask AI..." : "Message..."} style={{ flex:1, border:"none", background:"none", outline:"none", fontSize:13, color:"#1a1a1a" }} />
          {!isAI && <span style={{ color:"#999999", fontSize:14, cursor:"pointer" }}>📎</span>}
        </div>
...
// DMS SCREEN
// ══════════════════════════════════════════════════════════════════
export function DMSScreen({ go, docs }) {
  const [q, setQ] = useState("");
...
// ── DMS File ──────────────────────────────────────────────────────
export function DMSFileScreen({ go, ctx }) {
...
// STAFFING SCREEN
// ══════════════════════════════════════════════════════════════════
export function StaffingScreen({ go, clients, ctx }) {
  const [q, setQ] = useState("");
  const [expandedProcessStage, setExpandedProcessStage] = useState("Info Received");
  ...
  const processStageOrder = ["Preparation", "Awaiting Client Info", "Info Received", "QC Review", "Review", "Assembly", "Signer Sign-Off"];
  const processStageColors = {
    Preparation: "#6b7280",
    "Awaiting Client Info": "#4cb3e3",
    "Info Received": "#22c55e",
    "QC Review": "#f59e0b",
  ...
  const groupedProcessMap = new Map(processStageOrder.map((s) => [s, []]));
  rows.forEach((r) => {
    const c = byClientId.get(r.id);
    const step = Math.max(1, c?.step || 1);
    const stage = processStageOrder[Math.min(step - 1, processStageOrder.length - 1)];
    groupedProcessMap.get(stage)?.push({
      id: r.id,
      client: r.client,
      workflow: r.workflow,
      partner: r.partner,
  ...
  const processGroups = processStageOrder
    .map((stage) => ({
      stage,
      color: processStageColors[stage] || "#6b7280",
      items: (groupedProcessMap.get(stage) || []).sort((a, b) => {
        const riskScore = { high:3, med:2, low:1 };
        return (riskScore[b.risk] || 1) - (riskScore[a.risk] || 1) || b.step - a.step || a.client.localeCompare(b.client);
      }),
    }))
    .filter((g) => g.items.length > 0);
  const activeProcessStage = processGroups.some((g) => g.stage === expandedProcessStage)
    ? expandedProcessStage
    : processGroups[0]?.stage;
  const filteredClientIds = new Set(rows.map((r) => r.id));
  const reviewNotes = staffingReviewNotes
    .filter((n) => filteredClientIds.has(n.clientId))
    .filter((n) => {
      const query = q.trim().toLowerCase();
      ...
      {!isTableMode ? (
        <div style={{ padding:"10px 18px 20px", display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ display:"flex", gap:8, overflowX:"auto", WebkitOverflowScrolling:"touch", scrollbarWidth:"none", paddingBottom:2 }}>
            {[
              { key:"all", label:"All Returns", value:summary.total, bg:"#edf3ff", c:"#2f4d8f", bd:"#d6e3ff" },
              { key:"awaiting", label:"Awaiting", value:summary.awaiting, bg:"#fff7e7", c:"#b57913", bd:"#f6e2bb" },
              { key:"review", label:"Review", value:summary.review, bg:"#f6ecff", c:"#8640bf", bd:"#e4cef8" },
              { key:"risk", label:"At Risk", value:summary.atRisk, bg:"#ffefef", c:"#b45858", bd:"#f6cdcd" },
            ].map((m) => (
              <div
                key={m.key}
                style={{
              ...
              >
                <span style={{ fontSize:15, fontWeight:800, color:m.c, lineHeight:1 }}>{m.value}</span>
                <span style={{ fontSize:9.5, fontWeight:700, letterSpacing:0.45, textTransform:"uppercase", color:m.c, opacity:0.9, lineHeight:1 }}>
                  {m.label}
                </span>
              </div>
            ))}
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"2px 2px 0" }}>
            <div style={{ fontSize:18, fontWeight:700, color:"#1f2937" }}>Review notes</div>
            <div style={{ fontSize:12, color:"#7a818f" }}>{reviewNotes.filter((n) => n.status === "Open").length} open</div>
          </div>

          {reviewNotes.map((note) => {
            const linkedClient = clients.find((c) => c.id === note.clientId);
            const isOpen = note.status === "Open";
            return (
              <IOSCard key={note.id} padding="12px 12px 10px">
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:7, minWidth:0 }}>
                    <span style={{ fontSize:10, fontWeight:700, color:"#657086", textTransform:"uppercase" }}>{note.id}</span>
                    <span style={{ fontSize:11, fontWeight:700, color:"#0f766e", background:"#e9f6f3", border:"1px solid #bde9df", borderRadius:8, padding:"2px 7px" }}>{note.source}</span>
                  </div>
                  <span
                    style={{
                      fontSize:11,
                      fontWeight:700,
                      color:isOpen ? "#187d3f" : "#616b7b",
                      background:isOpen ? "#ecf9f1" : "#eff2f7",
                      border:`1px solid ${isOpen ? "#c7ebd4" : "#d8dde8"}`,
                      borderRadius:999,
                      padding:"3px 8px",
                      flexShrink:0,
                    }}
                ...
                <div style={{ fontSize:13.5, fontWeight:600, color:"#1f2937", marginTop:10, lineHeight:1.35 }}>
                  {note.description}
                </div>

                <div style={{ marginTop:10, borderTop:"1px solid #edf1f7", paddingTop:9 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:"#8a94a7", letterSpacing:1.1, textTransform:"uppercase", marginBottom:4 }}>Latest response</div>
                  <div style={{ fontSize:12.5, color:note.latestResponse ? "#4c5566" : "#8a94a7", lineHeight:1.35 }}>
                    {note.latestResponse || "No response yet"}
                  </div>
                  {note.latestMeta && (
                    <div style={{ fontSize:10.5, color:"#8a94a7", marginTop:3 }}>{note.latestMeta}</div>
                  )}
                </div>

                <div style={{ marginTop:10, display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                  <div style={{ minWidth:0 }}>
                    <div style={{ fontSize:10, fontWeight:700, color:"#8a94a7", letterSpacing:0.7, textTransform:"uppercase" }}>Reference</div>
                    <div style={{ fontSize:11.5, color:"#525b6c", marginTop:2, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{note.reference}</div>
                  </div>
                  <div style={{ minWidth:0 }}>
                    <div style={{ fontSize:10, fontWeight:700, color:"#8a94a7", letterSpacing:0.7, textTransform:"uppercase" }}>Created / Assigned</div>
                    <div style={{ fontSize:11.5, color:"#525b6c", marginTop:2 }}>{note.createdBy} {note.createdAt} · {note.assignedTo || "Unassigned"}</div>
                  </div>
                </div>

                <div style={{ display:"grid", gridTemplateColumns:"1fr auto", gap:8, marginTop:10 }}>
                  <button
                    onClick={() => linkedClient && go(S.CLIENT_DETAIL, { client: linkedClient, from:S.STAFFING })}
                    style={{ border:"none", borderRadius:11, background:"#0065FF", color:"#fff", fontSize:12, fontWeight:700, padding:"9px 10px", cursor:"pointer" }}
                  >
                    Open client
                  </button>
                  <button
                    style={{ border:"1px solid #d5dbe7", borderRadius:11, background:"#fff", color:"#4b5565", fontSize:12, fontWeight:700, padding:"9px 10px", cursor:"pointer" }}
                  >
                    Next note
                  </button>
                </div>
          ...
          {reviewNotes.length === 0 && (
            <IOSCard padding="12px">
              <div style={{ fontSize:11, color:"#8a8f9a" }}>No review notes for this filter.</div>
            </IOSCard>
          )}

          <IOSCard overflow="hidden">
            {processGroups.map((group, idx) => {
              const open = group.stage === activeProcessStage;
              return (
                <div key={group.stage} style={{ borderTop: idx === 0 ? "none" : "1px solid #eceef3" }}>
                  <button
                    onClick={() => setExpandedProcessStage(open ? "" : group.stage)}
                    style={{
                      width:"100%",
                      border:"none",
                      background:"#fff",
                  ...
                  >
                    <div style={{ display:"flex", alignItems:"center", gap:8, minWidth:0 }}>
                      <span style={{ width:8, height:8, borderRadius:99, background:group.color, flexShrink:0 }} />
                      <span style={{ fontSize:12.5, color:"#1f2937", fontWeight:700, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{group.stage}</span>
                    </div>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#8a93a3" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ transform: open ? "rotate(90deg)" : "rotate(0deg)", transition:"transform .15s ease" }}>
                      ...
                      {group.items.map((item, i) => (
                        <button
                          ...
                          style={{
                            ...
                            borderBottom: i === group.items.length - 1 ? "none" : "1px solid #f1f3f7",
                            background:"#fff",
                            padding:"9px 4px",
                            textAlign:"left",
                            display:"flex",
                        ...
                        >
                          <div style={{ minWidth:0, flex:1 }}>
                            <div style={{ fontSize:12, color:"#1f2937", fontWeight:700, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                              {item.client}
                            </div>
                            <div style={{ fontSize:10.5, color:"#6b7280", marginTop:1, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                              {item.workflow} · {item.partner}
                            </div>
                          </div>
                        </button>
                      ))}
            ...
            {processGroups.length === 0 && (
              <div style={{ padding:"12px", fontSize:11, color:"#8a8f9a" }}>No processes for this filter.</div>
            )}
          </IOSCard>

          <IOSCard padding="14px 14px 12px">
            <div style={{ fontSize:13, fontWeight:700, color:"#1f2937", marginBottom:4 }}>Load snapshot</div>
            <div style={{ fontSize:11, color:"#7a818f", marginBottom:10 }}>Capacity and bottleneck indicators</div>
          ...
          <IOSActionButton
            ...
            label="Open full staffing table"
          />
        </div>
      ) : (
...
// METRICS SCREEN
// ══════════════════════════════════════════════════════════════════
export function MetricsScreen({ go, clients }) {
